...
- **Mobile UI conventions**: safe-area insets are owned per top-level surface (app shell, drawer, artifact panel, onboarding) — never on `body`, since fixed surfaces escape body padding. Soft keyboard resizes the WebView (`Keyboard.resize: native` + `adjustResize`). Overlays register on `src/lib/back-stack.ts` via `useBackClose` so Android back closes them; `haptic()` in `src/lib/haptics.ts` is web-safe. Touch targets: button sizes extend an invisible `after:` hit area under `pointer-coarse:`; hover-only affordances need a `pointer-coarse:` fallback. Model picker uses a bottom `Sheet` below `md`. Settings is a sectioned full-screen page below `sm` (chips switch sections; desktop stacks all); the onboarding wizard is desktop-only — phones go straight to Settings.
...
- **Group chat / projects** — `worker/room.ts` is a `Room` Durable Object (one per room, SQLite-backed, WebSocket hibernation). It's a message relay only: the host's browser is the "runner" that calls the model with its own key and streams the reply back through the DO, which fans it out. Client: `src/lib/room.ts` (WS store + runner) drives `src/routes/room.tsx` (`/r/:token`). Rooms are server-authoritative (not Dexie). D1 `rooms` = registry, `room_members` = invite list. `join_mode` `guests` (link-open, guests join by `?name=`) vs `members` (invite-only project: host + invited GitHub logins only, matched at join). WS auth by cookie or `?token=` (WS can't set headers). Host controls model (labelled per reply), pause, close, and invites via the Invite dialog. Fixed `h-[100dvh]` flex layout: pinned header/composer, only the list scrolls, smart auto-scroll (stick-to-bottom only when already there). v1 is text-only (no tools/artifacts/attachments in group mode).
...
- **ChatGPT provider egress.** chatgpt.com's WAF hard-blocks Cloudflare Worker subrequests (Workers stamp an unremovable `cf-worker` header → bot-challenge 403, in local dev AND deployed) — and browser CORS is blocked too. The codex leg must egress from a non-Worker host: run `scripts/codex-forwarder.mjs` and point `CODEX_BASE_URL` at it (`.dev.vars` in dev, a var in prod; `CODEX_PROXY_SECRET` if the forwarder is public). auth.openai.com does NOT block Workers, so the OAuth proxies in `worker/routes/chatgpt.ts` work directly. Tool-loop reasoning items echo through `extra_content.chatgpt.items` — same opaque rail as Gemini's thought_signature.
